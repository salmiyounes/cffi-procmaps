...
def error(err: PROCMAPS_ERROR_T) -> Exception | None:
    msg: str = [
        "Failed to open the maps file (check /proc)", 
        "Failed to read from the maps file",        
        "Internal memory allocation (malloc) failed" 
    ][err]
...
def proc_map_iterator(procmaps_it ) -> Iterator["MemoryRegion"]: # type: ignore
    while ((mem_reg := lib.pmparser_next(procmaps_it)) != ffi.NULL): # type: ignore
        #pathname_raw = mem_reg.pathname
        #pathname = ffi.string(pathname_raw).decode('utf-8') if pathname_raw != ffi.NULL else None
        
        # Convert C char array to Python string
        #anon_name = ffi.string(mem_reg.map_anon_name).decode('utf-8')
...
class ProcMaps:
    ...
    def _initialize(self) -> PROCMAPS_ERROR_T:
        err: int = lib.pmparser_parse(self.pid, self.pointer)
        if err == PROCMAPS_SUCCESS:
            for m in proc_map_iterator(self.pointer):
                self.push(m)
